...
use crate::db::queries::{ChannelPost, ChannelSubscription, ChatMessage, FriendEntry};
use ratatui::{
...
pub fn render_app(f: &mut Frame, app_state: &AppState, ctx: &RenderContext) {
    ...
    let header_line = Line::from(vec![
        Span::styled(
            "  chattor",
            Style::default()
                .fg(ctx.theme.header_accent)
                .add_modifier(Modifier::BOLD),
        ),
        Span::styled(addr_display, Style::default().fg(ctx.theme.fg_dim)),
        Span::raw("  "),
        Span::styled(
            format!("{} {}", tor_icon, tor_label),
            Style::default().fg(tor_color),
        ),
    ]);
    ...
    match app_state {
        ...
        AppState::ViewingMyIdentity {
            ...
            crate::ui::modals::render_identity_modal(
                f,
                friend_code,
                onion_address,
                copied_field.as_deref(),
                &ctx.theme,
            );
        }
...
fn format_footer_spans<'a>(state: &AppState, theme: &'a Theme) -> Vec<Span<'a>> {
    let pairs: Vec<(&str, &str)> = match state {
        AppState::Normal {
            input_focused: true,
            ..
        } => vec![("Enter", "Send"), ("Esc", "Nav")],
        AppState::Normal { .. } => vec![
            ("Tab/\u{2191}\u{2193}", "Select"),
            ("Enter", "Open"),
            ("a", "Add"),
            ("n", "Notif"),
            ("s", "Subscribe"),
        ...
        AppState::ViewingFriendRequests { .. } => vec![
            ("\u{2191}\u{2193}", "Navigate"),
            ("Enter", "View"),
            ("Esc", "Back"),
        ],
        AppState::ViewingFriendRequest { .. } => {
            vec![("A", "Accept"), ("R", "Reject"), ("Esc", "Back")]
        }
        AppState::ViewingMyIdentity { .. } => {
            vec![("o", "Copy onion"), ("c", "Copy code"), ("i/Esc", "Close")]
        }
        AppState::SettingEphemeral { .. } => vec![
